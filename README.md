...
- **بحث حجوزات طيران حقيقي**: عبر Duffel (يتطلّب `DUFFEL_API_KEY`
  فعلياً، فارغ حتى الآن) — يعرض للعميل رحلات وأسعار حقيقية بين أي
  مدينتين، مع رسم حجز يضيفه المدير فوق السعر الحقيقي (فلات بالدولار أو
  نسبة مئوية، قابل للتعديل في أي وقت). الحجز الفعلي (إصدار التذكرة)
  يبقى يدوياً كما هو الحال في بقية الخدمات. كل طلب يتطلّب رقم واتساب
  للتواصل السريع مع العميل.
...
التصميم في `docs/UI_DESIGN_GUIDELINES.md`. **الهوية البصرية الرسمية**
(شعار الوكالة + لوحة كحلي/ذهبي على خلفية كريمية، مأخوذة من الشعار
الفعلي) مطبَّقة في كل الشاشات وإيميلات الإشعارات
(`frontend/public/logo.png`، ألوان `navy`/`gold`/`cream` في
`frontend/src/index.css`). **أولوية التصميم للهاتف
...
تشمل حالياً عشر شاشات كاملة: تسجيل الدخول (بريد/هاتف + كلمة مرور، أو
دخول سريع عبر قوقل)، لوحة التحكم الرئيسية (كروت KPI وجدول طلبات
تفاعلي)، الطلبات (إنشاء طلب جديد مع بحث رحلات طيران حقيقية لخدمات
التذاكر، عرض مستندات الخدمة المطلوبة، ورفع إثبات الدفع)، المدفوعات
(طابور مراجعة يدوية للموظف/المدير)، المستردات (طابور اعتماد/رفض/تنفيذ
للمدير)، الوكلاء (B2B)، الخدمات (كتالوج كامل مع إضافة/تعديل/حذف/عروض
خصم محدودة المدة، وإدارة قائمة مستندات كل خدمة)، العملات وسعر الصرف،
رسوم حجز الطيران (فلات/نسبة، admin)، وحسابي (بيانات شخصية وتغيير كلمة
المرور). دورة حياة
