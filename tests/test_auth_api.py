...
def test_register_customer_success(client):
    response = client.post(
        ...
        json={
            "full_name": "عميل جديد",
            "email": "new-customer@baradise.example",
            "phone": "0911111111",
...
def test_register_duplicate_phone_fails(client):
    payload = {
        "full_name": "عميل جديد",
        "email": "unique1@baradise.example",
        "phone": "0922222222",
        "password": "StrongPass@123",
    }
    ...
    payload["email"] = "unique2@baradise.example"
...
def test_login_success_and_wrong_password(client):
    client.post(
        ...
        json={
            ...
            "email": "login-user@baradise.example",
            "phone": "0933333333",
    ...
    good_login = client.post(
        "/api/v1/auth/login",
        json={"identifier": "login-user@baradise.example", "password": "CorrectPass@123"},
    )
    ...
    bad_login = client.post(
        "/api/v1/auth/login",
        json={"identifier": "login-user@baradise.example", "password": "WrongPassword"},
    )
...
def test_repeated_failed_logins_get_locked_out(client):
    client.post(
        ...
        json={
            ...
            "email": "lockout-user@baradise.example",
            "phone": "0944444444",
            "password": "CorrectPass@123",
        },
    )
    ...
    for _ in range(5):
        response = client.post(
            "/api/v1/auth/login",
            json={"identifier": "lockout-user@baradise.example", "password": "WrongPassword"},
        )
        assert response.status_code == 401

    locked_response = client.post(
        "/api/v1/auth/login",
        json={"identifier": "lockout-user@baradise.example", "password": "CorrectPass@123"},
    )
...
def test_password_change_invalidates_old_token_and_issues_new_one(client):
    client.post(
        ...
        json={
            ...
            "email": "password-change-user@baradise.example",
            "phone": "0955555555",
    ...
    login_response = client.post(
        "/api/v1/auth/login",
        json={"identifier": "password-change-user@baradise.example", "password": "OldPass@123"},
    )
